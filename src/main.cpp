...
const String congratulation = "chuc co 20.10 vui ve";
...
void drawHeartText(bool isDisplay = true)
{
  ...
  int16_t class_x = SCREEN_WIDTH / 2 - HEART_WIDTH / 2 -class_w - TEXT_HEART_OFFSET;
  int16_t class_y = SCREEN_HEIGHT / 2 - int(class_h / 2);
  int16_t prof_x = SCREEN_WIDTH / 2 + HEART_WIDTH / 2 + TEXT_HEART_OFFSET;
  ...
  display.drawBitmap(
      ...
      currentHeartBitmap, HEART_WIDTH, HEART_HEIGHT, SSD1306_WHITE);
  if (isDisplay)
  {
    display.display();
  }
...
void blitOr(const uint8_t *src, int src_w, int src_h, int dest_x, int dest_y)
{
  ...
  // entirely off-screen
  if (dest_x + src_w <= 0 || dest_x >= buf_w)
    return;
  if (dest_page + src_pages < 0 || dest_page >= (buf_h / 8))
    return;
  ...
  for (int x = 0; x < src_w; ++x)
  {
    ...
    for (int p = 0; p < src_pages; ++p)
    {
      ...
      if (dest_index < 0 || dest_index >= buf_size)
        continue;
...
void splitScreen()
{
  ...
  for (uint16_t i = 1; i <= SCREEN_HEIGHT / 2; i++)
  {
    ...
    display.drawPixel(SCREEN_WIDTH/2, 0, WHITE);
    display.display();
    delay(100);
  }
}
    ...
    if (millis() - start < duration_ms)
    {
      if (now - lastSpawn >= spawnInterval)
      {
        ...
        for (int i = 0; i < leftBurst; ++i)
          spawnParticleFromLeft();
        for (int i = 0; i < rightBurst; ++i)
          spawnParticleFromRight();
      }
    }
